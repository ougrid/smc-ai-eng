...
*(v2.7 — **hybrid retrieval promoted from Future-improvements into the agent** (post-Day-5 hardening): `agent/text_search_tool.py` adds a Postgres full-text (`tsvector`/`ts_rank_cd`) lexical search over 10-K chunk text, seeded from the same `data/pinecone_vectors.jsonl.gz` source as Pinecone (new `chunk_text` table, loaded by `scripts/load_chunk_text.py`); `agent/hybrid_tool.py` fuses it with the existing dense (Pinecone) ranking via reciprocal rank fusion (`agent/fusion.py`) before the existing boilerplate/dedupe filters run. Mitigates the dense-only miss on exact terms (ticker symbols, "Item 1A", named products) named in the roadmap below. Fails open: if `chunk_text` doesn't exist yet (pre-hybrid Postgres volume), retrieval silently degrades to dense-only rather than erroring — no feature flag needed. v2.6 — **history-aware routing/synthesis** (post-Day-5 hardening, found via live multi-turn testing): the route and synthesize nodes now consume a capped verbatim window of the conversation (`HISTORY_MAX_MESSAGES`) so elliptical follow-ups ("the revenue", "drill down further") resolve against prior turns instead of re-triggering `clarify` every time; this closes a gap between the design (item 8 below always assumed the router "sees" history) and the actual Day-3 implementation, which never read `state["history"]` at all. Deliberately **verbatim, not summarized** — see the new Future-improvements entry on hierarchical summarization for why that's deferred. v2.5 — **intent gate + clarify path added to the agent**: route node now classifies question intent (financial / off-topic / vague) — off-topic gets a polite scope refusal, vague gets a clarifying question back to the user; company mentions are normalized via the LLM's own brand knowledge (world-knowledge entity resolution) with the alias table demoted to deterministic backstop — less manual alias config in production. Companion doc: `agent-output/technical-execution-plan.md` (code-level execution detail). v2.4 — **streaming promoted into v1** (SSE, AI SDK UI message stream protocol from FastAPI); frontend stack pinned: AI SDK `useChat` + shadcn/ui; Auth.js evaluated and rejected for v1 (custom FastAPI JWT stands — rationale in Auth §); template-inspired future improvements (AI SDK OTel telemetry, WebSocket transport, resumable streams). v2.3 — structured-output enforcement specified from verified July-2026 research (`agent-output/structured-output-research.md`): strict json_schema via `with_structured_output`, reasoning-first schema, in-node re-ask → fail-closed. v2.2 — folded in learnings from prior `spw_retail-rag` project (see `agent-output/spw-retail-rag-learnings.md` §A): DI app factory, numeric-consistency `verify` node promoted from future→Day 4, fail-closed routing, fixture-driven routing tests, `debug` payload, typed config knobs, optional compose seed profile. v2.1 — merged with Ultraplan cloud refinements: read-only `agent_ro` role, describe_index host resolution, metadata junk-key cleanup, boilerplate-chunk handling, graph diagram. Added: budget fallback, production-hardening roadmap)*
...
scripts/
  initdb/
    ...
    02_chunk_text.sql       # chunk_text table (tsvector + GIN idx) — lexical half of hybrid retrieval
    03_roles.sql            # CREATE ROLE agent_ro; GRANT SELECT ON financial_data + chunk_text ONLY
  ...
  load_chunk_text.py        # same source file → Postgres chunk_text, idempotent (ON CONFLICT DO NOTHING)
...
backend/
  ...
  app/
    ...
    config.py               # pydantic-settings; every tuning knob (score floor, top_k,
                            #   history window, model names, agent_ro DSN) typed + env-configurable
    db.py
    ...
    agent/   state.py · graph.py (build_graph(sql_tool, vector_tool, llm) — clients
             ...
             sql_tool.py · vector_tool.py · text_search_tool.py · hybrid_tool.py · fusion.py
...
- ~~Hybrid retrieval + reciprocal-rank fusion~~ — **promoted post-Day-5** (`agent/text_search_tool.py`, `agent/fusion.py`, `agent/hybrid_tool.py`, new `chunk_text` Postgres table): dense (Pinecone) + lexical (Postgres full-text) rankings fused per company via RRF before the existing boilerplate/dedupe filters. Fixes the dense-only miss on exact-term matches (ticker symbols, "Item 1A", specific product names). Not yet done: reranking the fused pool (next bullet) and BM25 proper (this uses Postgres `ts_rank_cd`, a weaker but zero-new-infra stand-in for true BM25).
- **Cross-encoder reranking** — rerank the fused top-30 down to top-6 with a reranker (bge-reranker-v2-m3 locally, or a hosted rerank API). Biggest remaining precision win available for chunk quality; directly mitigates the print-to-PDF boilerplate chunks. Natural next step now that hybrid fusion produces a wider, better-recalled candidate pool to rerank.
...
- **Chat history persistence** — `messages(id, session_id, user_id FK, role, content, route, citations, created_at)` table, written once the SSE stream completes (not mid-stream, so a dropped connection never leaves a half-written turn); backs `GET /api/chat/sessions` + `GET /api/chat/sessions/{id}` for cross-device continuity and is the storage prerequisite for the resumable-streams item below. v1 is deliberately stateless (history lives only in the browser tab, rebuilt from `useChat`'s in-memory state) — this is the natural first "make it durable" step before reaching for Redis/resumable streams.
- **Hierarchical conversation summarization** — the capped verbatim window the route/synthesize nodes use today (`HISTORY_MAX_MESSAGES`, see Agent design item 1) is sized for this app's actual usage: a financial Q&A session is a handful of short exchanges, not a long-running agentic conversation. If usage patterns ever change (very long sessions, embedded/multi-hour use), the standard escalation is a three-tier memory, not just a bigger window: (a) a **verbatim window** of the most recent 3–6 turns, always kept as-is, for natural short-range continuity; (b) an **LLM-generated summary** of everything older than that window, condensed into a compact digest and regenerated incrementally (summarizing only the newly-aged-out turns each time, not re-summarizing the whole history) once the conversation crosses a size threshold — the common rule of thumb is ~80–85% of the model's context window, or roughly 15–20 exchanges for a shorter-context model; (c) **final prompt assembly** = system instructions + the historical summary + the verbatim recent turns + the new message. Two things worth flagging before reaching for this: LangChain doesn't ship a polished one-line summarizer for it — `trim_messages` only trims by token/message count, not semantic compaction — so this would be a genuine custom LangGraph node (a summarize step gated on a token-count trigger), not a config flag; and a summarizer is itself a new hallucination surface — a lossy summary that misstates a number becomes a "fact" the router/synthesizer then treats as ground truth, which cuts directly against this project's core no-hallucination guarantee. Given the current window already fully covers real follow-up questions at negligible token cost, this is deliberately deferred rather than built pre-emptively.
