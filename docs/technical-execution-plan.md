...
*(Companion to `implementation-plan-v2.md` (strategy, v2.11). This is the code-level detail layer: concrete enough that implementation needs zero design pauses. Execution decisions made here: canonical company keys = **SQL names** (`Google`, `Meta`, `Apple`, `Amazon`); `agent_ro` fixed dev password in plain SQL (initdb `.sql` has no env substitution); verify-veto mechanics = **"render only the last text part"** frontend rule; synthesize streaming = **server-side incremental extraction of the `answer` field** from the strict-JSON token stream (`AnswerFieldExtractor`); intent gate lives inside the route node's structured output (no extra LLM call) — a 4th intent, `capability`, answers meta/onboarding questions about the assistant's own scope via a deterministic node (`agent/nodes/capability.py`) built from the live `CoverageMap`, so onboarding questions like "what data do you have?" never hit the off_topic refusal template; conversation history threaded into route + synthesize as a **capped verbatim window** (`HISTORY_MAX_MESSAGES`, `agent/history.py`, default 10) — never summarized, see the implementation plan's Future-improvements entry for why; `chat/schemas.py`'s `_text_of` keeps only the **last** text part per message, so a stream-veto's discarded fabricated draft never resurfaces when that message is replayed as history; **hybrid retrieval** (post-Day-5) fuses dense (Pinecone) and lexical (Postgres full-text, new `chunk_text` table) rankings per company via reciprocal rank fusion, chunk ids shared across both stores so fusion is a plain id join, no separate mapping layer; **cross-encoder reranking** (post-Day-5) widens that same per-company retrieval to `RERANK_POOL_SIZE` before the fusion cut, then reranks and re-cuts down to `TOP_K` with a local `bge-reranker-v2-m3` model -- chosen over a hosted rerank API to keep the OpenAI-only stack decision intact; **RAGAS eval suite** (post-Day-5, `scripts/eval_ragas.py`) lives behind an optional `eval` dependency group specifically so the core app/test suite never touch `pyarrow` (via `ragas`'s `datasets` dependency) -- see README for a real Windows Application Control incident this avoids repeating for anyone else.)*
...
SCORE_FLOOR=0.25
TOP_K=10                                    # final evidence count per company, post-rerank
SQL_ROW_LIMIT=100
HISTORY_MAX_MESSAGES=10                     # capped verbatim window fed to route + synthesize; see history.py
RERANK_POOL_SIZE=30                         # pre-rerank retrieval breadth; see reranked_tool.py
...
- **`agent/reranker.py`** (post-Day-5): `CrossEncoderReranker(model_name).score(question, texts) -> list[float]` — thin wrapper over `sentence_transformers.CrossEncoder(model_name).predict([(question, text) for text in texts])`. `CrossEncoder` is imported at module level specifically so tests can `unittest.mock.patch("app.agent.reranker.CrossEncoder", ...)` with a stub — the real `BAAI/bge-reranker-v2-m3` model (~1GB) downloads from Hugging Face on first real construction, which only happens in `main.py`'s `_build_real_graph`, never in a test.
- **`agent/reranked_tool.py`** (post-Day-5): `RerankedTool(inner, reranker, *, top_k).query(question, companies) -> VectorResult` — wraps any `VectorQueryable` (in practice `HybridTool` configured with a wide `rerank_pool_size`); per company (never pooled across companies, same skew reasoning as `HybridTool`), scores every candidate chunk's text against the question via `Reranker.score`, keeps the top `top_k` by that score, cuts the rest with `rejected` reason `"reranked_out"`. Fail-open: a `Reranker.score` exception falls back to the inner tool's own ordering rather than crashing the node, but the `top_k` cut still applies either way — a reranker outage can't silently balloon the evidence pool back up to `rerank_pool_size`.
...
Makefile: `up` = `docker compose up -d` · `seed` = `uv run --project backend python scripts/load_pinecone.py && uv run --project backend python scripts/load_chunk_text.py` (post-Day-5: two loaders, same source file, same idempotency contract) · `api` = `uvicorn app.main:app --reload --port 8000` · `web` = `npm --prefix frontend run dev` · `test` = pytest · `eval` = eval_baseline · `eval-ragas` (post-Day-5) = `uv run --project backend --extra eval python scripts/eval_ragas.py` (`--extra eval` is load-bearing -- without it `uv run` re-syncs to the default dependency set and silently strips `ragas` back out) · `down` = `docker compose down -v` (**-v is load-bearing**: forces initdb re-run). README lists raw commands beside each target (Windows without make). Scripts run under the backend uv project — one environment.
...
| `test_reranker.py` (new, post-Day-5) | patches `app.agent.reranker.CrossEncoder` with a stub (no real ~1GB model ever downloaded in tests) — scores returned as floats in text order; question paired with every text; empty text list skips `predict` entirely; `model_name` passed through to the underlying `CrossEncoder` constructor |
| `test_reranked_tool.py` (new, post-Day-5) | no chunks / no companies → inner result passed through unchanged (identity); reranker reorders and cuts to `top_k`, cut chunks get `rejected` reason `"reranked_out"`; existing `rejected` reasons from the inner tool are carried through; companies reranked independently (each gets its own `top_k` cut); reranker exception → falls back to the inner tool's own ordering but still applies the `top_k` cut (fail-open, never unbounded) |
...
- **Post-Day-5 hardening (cross-encoder reranking)**: `build: add sentence-transformers dependency` → `feat(agent): add reranker and reranked_tool, wire into main.py` → `test(agent): reranker, reranked tool` → `docs: promote cross-encoder reranking from roadmap`
- **Post-Day-5 hardening (RAGAS eval suite)**: `build(backend): add ragas as an optional eval dependency group` → `feat(scripts): add RAGAS quality-scoring eval script` → `docs: promote RAGAS eval suite from roadmap, add Windows Application Control troubleshooting note`
